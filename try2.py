...
train_path = "/data/qdg/TACO-main-copy/data/ED/train.csv"
valid_path = "/data/qdg/TACO-main-copy/data/ED/valid.csv"
test_path  = "/data/qdg/TACO-main-copy/data/ED/test.csv"
...
batch_size = 64
grad_accum_steps = 1      # ✅ 有效 batch=32，更稳

max_len = 128             # ED 文本一般更长，128 合理（显存紧可改 96）
warmup_ratio = 0.05       # ✅ 推荐默认；对照可用 0.03

freeze_top_k = 4          # ✅ 比 2 更稳（显存紧可改回 2）
freeze_epochs = 2         # 前 2 个 epoch 只训 top-k + head

# 长尾损失（更温和组合）
use_focal = True
focal_gamma = 0.5
focal_weight = 0.05       # ✅ 比 0.2 更温和，通常更稳
...
# --------------------------
# 冻结/解冻（只改 requires_grad，不重建 optimizer）
# --------------------------
...
def set_classifier_trainable(hf_model, trainable=True):
    # HF sequence classification 常见为 .classifier
...
# ✅ optimizer 不重建：直接包含所有 multi_model 参数
...
for epoch in range(multi_epochs):
    # 解冻点：epoch==freeze_epochs 时解冻全部 encoder
    ...
    # early stop / save best by macro-F1
